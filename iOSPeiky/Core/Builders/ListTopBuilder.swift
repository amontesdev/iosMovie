...
class ListTopBuilder: ObjectBuilder{
    var builder: Builder?
    ...
    func build<T>(from dictionary: [[String : Any]]?) -> T? {
        var listTopResponse = ListTopCellResponse()
        if let listTopArray = dictionary! as? [[String:Any]]{
            var topMovies = [ListTopModel]()
            listTopArray.forEach{(item) in
                var movie = ListTopModel()
            ...
            listTopResponse.results = topMovies
            
        }
        return listTopResponse as? T
